...
# OMR
OMR.act <- read.csv(file.path(action_path,'demo_action_inputs/IBMR_OMR_SummerFallX2VOI_input.csv'),header=T)
...
# Temp
Temp.act <- read.table(file.path(action_path,'demo_action_inputs/Wetlands_2_hi_temp_temp.txt'),header=F)
...
# Secchi
secchi.act <- read.table(file.path(action_path,'demo_action_inputs/AWC_Yolo_secchi.txt'),header=F)
for (y in 1:(n.years+1)) {
for (m in 1:12) {
...
# X2
X2.dist.act <- read.csv(file.path(action_path,'demo_action_inputs/IBMR_X2_SummerFallX2VOI_input.csv'),header=T)
X2.dist.act <- X2.dist.act[which(X2.dist.act==paste0(action)),]
rownames(X2.dist.act) <- X2.dist.act[,2]
X2.target <- X2.dist.act[,3:14]

# Prey
PD.act <- read.csv(file.path(action_path,'demo_action_inputs/zoop_scalar_output_2025-03-10.csv'),header=T)
